-- HB rework: remove streak multiplier, remove flat speed bonus,
-- add improvement bonus (1 HB per full second your avg top-10 drops)
...
BEGIN
  ...
  IF NOT p_offline AND v_old_avg_ms IS NOT NULL THEN
    ...
    IF v_new_avg_ms IS NOT NULL AND v_new_avg_ms < v_old_avg_ms THEN
      v_improvement_ms := v_old_avg_ms - v_new_avg_ms;
      v_speed_bonus := GREATEST(floor(v_improvement_ms / 1000.0), 1);
    END IF;
  END IF;
  ...
  -- Insert speed bonus transaction
